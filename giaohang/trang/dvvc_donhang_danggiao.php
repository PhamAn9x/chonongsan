...
<div id="cho_id">
    ...
    <article style="width: 98%; margin-left: 1%; padding-top: 3%;">
      <main  id="table" class="table-editable">
        ...
        <div class="panel panel-default">
          ...
          <div class="panel-heading" style="text-align: center; font-size: 22px; text-transform: uppercase;">đơn hàng đang giao</div>
 ...
 -->
      ...
      <table id="example2" class="table searchable table-striped table-hover">
        <thead>
          <tr>
            ...
            <th>Trạng thái</th>
            <!-- <th style="width:20px;">Không nhận</th> -->
          </tr>
        </thead>
           <?php
    session_start();
    ...
    $sql = "SELECT * FROM DONHANG WHERE DH_DVVC_ID = '$dvvc' AND DH_TRANGTHAI  = 4";
    ?>
        <tbody>
          <?php
        $stt=1;
        ...
        while( $row = mysqli_fetch_array($rs,MYSQLI_ASSOC))
        {
            ...
            <tr>
                ...
                <td>
                    <input style="display: none;" type="text" id="trangthai<?php echo $row['DH_ID']; ?>" value="<?php echo $row['DH_ID']; ?>">
                    <style type="text/css">
                        select {
                                       text-align: center;
                  ...
                  <select  class="" id="sltrangthai<?php echo $row['DH_ID']; ?>">
                   ...
                   <div style="display: none;" id="them<?php echo $row['DH_ID']; ?>">
                 <select style="width: 185px;" id="sllydo<?php echo $row['DH_ID']; ?>">
                    <option value="">Chọn lí do</option>
                     <option value="Không tìm được địa chỉ"> Không tìm đưọc địa chỉ </option>
                     <option value="Người nhận không nhận hàng"> Ngưòi nhận không nhận hàng</option>
                      <option value="Ngưòi nhận vắng nhà"> Ngưòi nhận vắng nhà</option>
                   </select>
                </div>
                </td>
            ...
            <script type="text/javascript">
                $(document).ready(function(){
                    $("#sltrangthai<?php echo $row['DH_ID']; ?>").change(function(){
                        ...
                        if(tt == 5){
                          document.getElementById("them<?php echo $row['DH_ID']; ?>").style.display = "inherit"; 
                        } else{
                          var id = $('#trangthai<?php echo $row['DH_ID']; ?>').val();
                        var tt = $('#sltrangthai<?php echo $row['DH_ID']; ?>').val();
                        $.post("../process_ajax/ajax_xuly_donhangchonhan.php", {xlttdg:1,id: id,tt:tt}, function(data){
                            $("#cho_id").html(data);
                        });
                        }
                    });

                    $("#sllydo<?php echo $row['DH_ID']; ?>").change(function(){
                        var id = $('#trangthai<?php echo $row['DH_ID']; ?>').val();
                        var tt = $('#sltrangthai<?php echo $row['DH_ID']; ?>').val();
                        var ld = $('#sllydo<?php echo $row['DH_ID']; ?>').val();
                        $.post("../process_ajax/ajax_xuly_donhangchonhan.php", {xlttkg:1,id: id,tt:tt,ld:ld}, function(data){
                            $("#cho_id").html(data);
                        });
                    })
