            ...
            <div id="output">
                <table class="w3-table-all w3-hoverable w3-large tb">
                    ...
                    <?php 
                        ...
                        $sdt = $_SESSION['sdt'];
                        $sql = "SELECT * FROM LENH WHERE L_TEN ='ban' AND L_SDT ='$sdt'";
                        $rs = mysqli_query($conn,$sql);
                        $i=1;
                        if(mysqli_num_rows($rs)<1){
                            ?>
                            <tr>
                                <td colspan="9"> <i> Bạn chưa đặt lệnh bán!</i></td>
                            </tr>
                            <?php
                        } else
                        foreach ($rs as $value) {
                    ...
                    <script type="text/javascript">
                            $(document).ready(function(){
                                $("#delete<?php echo $value['SP_ID']; ?>").click(function(){
                                    ...
                                    if(confirm('Bạn có chắc muốn xóa lệnh bán này?')){
                                   $.post("process_ajax/xoa_lenhban.php", {key: id, sdt : sdt}, function(data){
                                        $("#alert").html(data);
                                    });
                               }
                                });
...
<script type="text/javascript">
    $(document).ready(function(){
         $("#timkiem").click(function(){
            var key = $("#ip").val();
            $.post("process_ajax/data_xuly_timkiem_lenh.php", {keys: key}, function(data){
                $("#output").html(data);
            });
        });
    });
