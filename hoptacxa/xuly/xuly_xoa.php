<?php
	...
	if(isset($_POST['block_multi'])){
			...
			for($i=0;$i< count($arr_sp);$i++){
				$sp = $arr_sp[$i];
				if(
					mysqli_query($conn,"DELETE FROM SANPHAM WHERE SP_ID = $sp") 
					&& mysqli_query($conn,"DELETE FROM HINHANH WHERE SP_ID = $sp") 
					&&  mysqli_query($conn,"DELETE FROM BINH_LUAN WHERE SP_ID = $sp" )
						&&  mysqli_query($conn,"DELETE FROM LENH WHERE SP_ID = $sp"  )){
				 $check=0;
				} else $check = 1;
			}
			if($check==0){
		...
		$sp_id = $_POST['xoa_sp'];
		if(
			mysqli_query($conn,"DELETE FROM SANPHAM WHERE SP_ID = $sp_id") && 
			mysqli_query($conn,"DELETE FROM HINHANH WHERE SP_ID = $sp_id") &&  
			mysqli_query($conn,"DELETE FROM LENH WHERE SP_ID = $sp_id"  )&&
			mysqli_query($conn,"DELETE FROM BINH_LUAN WHERE SP_ID = $sp_id" )
